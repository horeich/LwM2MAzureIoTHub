...
public class HttpClient implements IHttpClient {
    ...
    private static void setTimeout(IHttpRequest request, HttpRequestBase httpRequest) {
        RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(request.getOptions().getTimeout())
                .build();
    ...
    private CompletionStage<IHttpResponse> execute(IHttpRequest request, HttpMethod httpMethod) {
        try (CloseableHttpClient httpclient = HttpClients.createDefault()) {
            HttpRequestBase httpRequest = (HttpRequestBase) requestMap.get(httpMethod).getConstructor(URI.class)
                    .newInstance(request.getUri());
            ...
            log.debug(String.format("Sending request %s %s ", httpMethod.getName(), request.getUri().toString()));
            ...
            log.debug(String.format("Sending request %s %s :content:%s", httpMethod.getName(),
                    request.getUri().toString(), content));
